...
public class ClientGui implements Assignment3Starter.OutputPanel.EventHandlers {
	...
	public void submitClicked() {
		...
		else if(currentMessage.equals("greetings"))
		{
			request.put("type", "questions"); 
			request.put("questions", input); 
		}
		else if(currentMessage.equals("ready?"))
		{
			request.put("type", "startGame"); 
			request.put("response", input); 
		}
		else if(currentMessage.equals("gameTime"))
		{
			request.put("type", "answer");
			request.put("answer", input); 
		}
		else if(currentMessage.equals("gameOver"))
		{
			request.put("type", "restart"); 
			request.put("restart", input); 
		}
		else if(currentMessage.equals("continue"))
		{
			request.put("type", "restart"); 
			request.put("restart", input); 
		}
		else if(currentMessage.equals("goodbye"))
		{
			request.put("type", "finished"); 
			request.put("finished", input); 
		}
		else
		{
			request.put("type", "error"); 
			request.put("error", input); 
		}
		...
		try {
			...
			JSONObject json = new JSONObject(string);
			outputPanel.appendOutput(json.getString("value")); //puts new message from server in GUI
			outputPanel.clearInputText(); 
			if(json.has("value2"))
			{
				outputPanel.appendOutput(json.getString("value2"));
			}
			if(json.has("value3"))
			{
				outputPanel.appendOutput(json.getString("value3")); 
			}
			if(json.has("image"))
			{
				insertImage(json.getString("image"), 0, 0); 
			}
			currentMessage = json.getString("type"); //saves type for next submitClicked() loop
		} catch (Exception e) {
			e.printStackTrace();
		}
		close();
